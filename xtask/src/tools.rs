...
fn run_install_fallback(args: Vec<String>) -> Result<()> {
    let mut opts = InstallOptions {
        work_dir: PathBuf::from("/tmp/install-with-fallback"),
        ...
        match args[i].as_str() {
            "--name" => {
                opts.name = args
                    .get(i + 1)
                    .cloned()
                    .ok_or_else(|| anyhow::anyhow!("--name requires value"))?;
                i += 2;
            }
            "--check-cmd" => {
                opts.check_cmd = args
                    .get(i + 1)
                    .cloned()
                    .ok_or_else(|| anyhow::anyhow!("--check-cmd requires value"))?;
                i += 2;
            }
            "--apt" => {
                opts.apt.push(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--apt requires value"))?,
                );
                i += 2;
            }
            "--dnf" => {
                opts.dnf.push(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--dnf requires value"))?,
                );
                i += 2;
            }
            "--yum" => {
                opts.yum.push(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--yum requires value"))?,
                );
                i += 2;
            }
            "--pacman" => {
                opts.pacman.push(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--pacman requires value"))?,
                );
                i += 2;
            }
            "--apk" => {
                opts.apk.push(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--apk requires value"))?,
                );
                i += 2;
            }
            "--brew" => {
                opts.brew.push(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--brew requires value"))?,
                );
                i += 2;
            }
            "--winget" => {
                opts.winget.push(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--winget requires value"))?,
                );
                i += 2;
            }
            "--scoop" => {
                opts.scoop.push(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--scoop requires value"))?,
                );
                i += 2;
            }
            "--choco" => {
                opts.choco.push(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--choco requires value"))?,
                );
                i += 2;
            }
            "--prebuilt-url" => {
                opts.prebuilt_url = Some(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--prebuilt-url requires value"))?,
                );
                i += 2;
            }
            "--prebuilt-target" => {
                opts.prebuilt_target =
                    Some(PathBuf::from(args.get(i + 1).cloned().ok_or_else(
                        || anyhow::anyhow!("--prebuilt-target requires value"),
                    )?));
                i += 2;
            }
            "--source-url" => {
                opts.source_url = Some(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--source-url requires value"))?,
                );
                i += 2;
            }
            "--source-build" => {
                opts.source_build = Some(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--source-build requires value"))?,
                );
                i += 2;
            }
            "--source-install" => {
                opts.source_install = Some(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--source-install requires value"))?,
                );
                i += 2;
            }
            "--work-dir" => {
                opts.work_dir = PathBuf::from(
                    args.get(i + 1)
                        .cloned()
                        .ok_or_else(|| anyhow::anyhow!("--work-dir requires value"))?,
                );
                i += 2;
            }
            "--dry-run" => {
        ...
        match args[i].as_str() {
            "-i" | "--input" => {
                input = args
                    .get(i + 1)
                    .cloned()
                    .ok_or_else(|| anyhow::anyhow!("--input requires value"))?;
                i += 2;
            }
            "-o" | "--output" => {
                output = Some(
            ...
            "-t" | "--tool" => {
                tool = args
                    .get(i + 1)
                    .cloned()
                    .ok_or_else(|| anyhow::anyhow!("--tool requires value"))?;
                i += 2;
            }
            "-s" | "--strip" => {
                strip = true;
    ...
    match selected_tool.as_str() {
        ...
        "bash-obfuscate" => {
            if command_exists("bash-obfuscate") {
                run_checked(
                    Command::new("bash-obfuscate")
                        .arg(&input)
                        .arg("-o")
                        .arg(&out),
                    "bash-obfuscate protect",
                )?;
            } else {
                run_checked(
                    Command::new("npx")
